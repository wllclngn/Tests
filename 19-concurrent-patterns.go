...
func main() {
    ...
    go func(x chan string) {
        ...
        for j := 0; j < 50; j++ {
            foundMan := <-x
            if j < 15 {
                fmt.Println("#", j, ": GOT a " + foundMan + ". It's outdated!")
            } else if j < 25 {
                fmt.Println("#", j, ": GOT a " + foundMan + ". Yellow pages, and rats have ate on it.")
            } else {
                fmt.Println("#", j, ": GOT a " + foundMan + ". Almost need a break.")
            }
        }
    }(manuals)
    ...
    go func(y chan string) {
        ...
        for k := 0; k < 50; k++ {
            foundMan2 := <-y
            if k < 15 {
                fmt.Println("#", k, ": FOUND a " + foundMan2 + "! Yah there, bud.")
            } else if k < 25 {
                fmt.Println("#", k, ": FOUND a " + foundMan2 + ". You betcha.")
            } else {
                fmt.Println("#", k, ": FOUND a " + foundMan2 + ". Workin' up a sweat!")
            }
        }
    }(manuals2)
